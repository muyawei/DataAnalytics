...
ignorewords = {'the': 1, 'of': 1, 'to': 1, 'and': 1, 'a': 1, 'in': 1, 'is': 1,
               'it': 1}
            ...
            for (urlid,) in self.con.execute('select rowid from urllist'):
                ...
                for (linker,) in self.con.execute(
                    ...
                    linkingpr = self.con.execute(
                        'select score from pagerank where urlid=%d' % linker).fetchone()[
                        0]

                    # Get the total number of links from the linker
                    linkingcount = self.con.execute(
                        'select count(*) from link where fromid=%d' % linker).fetchone()[
                        0]
...
class searcher:
    ...
    def inboundlinkscore(self, rows):
        ...
        inboundcount = dict([(u, self.con.execute(
            'select count(*) from link where toid=%d' % u).fetchone()[0]) for
                             u in uniqueurls])
        ...
        for wordid in wordids:
            ...
            for (fromid, toid) in cur:
                if toid in linkscores:
                    pr = self.con.execute(
                        'select score from pagerank where urlid=%d' % fromid).fetchone()[
                        0]
                    linkscores[toid] += pr
    ...
    def pagerankscore(self, rows):
        pageranks = dict([(row[0], self.con.execute(
            'select score from pagerank where urlid=%d' % row[0]).fetchone()[
            0]) for row in rows])
